...
fun main(args: Array<String>) {
    val quizList: List<Quiz> = arrayListOf(
        Quiz(
            id = 1,
            category = "영어",
            level = 1,
    ...
    val csvFilePath = "quiz_data.csv"

    try {
        CSVWriter(FileWriter(csvFilePath)).use { csvWriter ->
            val headerRecord = arrayOf(
                "id", "category", "level", "imageUrl", "answer", "question", "choiceList",
                "time", "chance", "reward", "description"
            )
            csvWriter.writeNext(headerRecord)
            for ((id, category, level, imageUrl, answer, question, choiceList, time, chance, reward, description) in quizList) {
                val dataRecord = arrayOf(
                    id.toString(),
                    category,
                    level.toString(),
                    imageUrl,
                    answer.toString(),
                    question,
                    java.lang.String.join("|", choiceList),
                    time.toString(),
                    chance.toString(),
                    reward.toString(),
                    description
            ...
            println("Data has been written to $csvFilePath")
        }
    } catch (e: IOException) {
        e.printStackTrace()
    }
